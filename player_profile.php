...
<head>
    ...
    <meta name="description" content="Manage your ArenaReserve player profile — update your name, email, city, phone and profile picture.">
...
<body class="bg-slate-50 min-h-screen flex flex-col">
                ...
                <div class="profile-card rounded-2xl p-8 shadow-sm">
                    ...
                    <div class="flex flex-col items-center mb-8">
                        <div class="relative">
                            ...
                            <!-- Edit button -->
                            <button type="button" onclick="document.getElementById('avatarInput').click()" class="avatar-edit-btn" title="Change profile picture" id="avatarEditBtn">
    ...
    <div id="cropperModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-100 animate-[fadeIn_0.2s_ease-out]">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-base font-bold text-slate-800">Crop Profile Picture</h3>
                <button type="button" onclick="closeCropperModal()" class="text-slate-400 hover:text-slate-600 focus:outline-none transition-colors p-1 rounded-full hover:bg-slate-100">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="p-6">
                <div class="max-h-[300px] overflow-hidden rounded-xl border border-slate-200 bg-slate-100 flex items-center justify-center">
                    <img id="cropperImage" src="" alt="To Crop" class="max-w-full block">
                </div>
            </div>
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onclick="closeCropperModal()" class="px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
                <button type="button" id="applyCropBtn" class="px-4 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-sm transition-colors">Apply Crop</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // ── Avatar file input → live preview with Cropper ────────────────────
        ...
        applyCropBtn.addEventListener('click', function () {
            ...
            avatarImg.src = canvas.toDataURL('image/jpeg', 0.9);
